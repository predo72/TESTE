...
ReportBuilder Enterprise v. 15.02 Delphi7
Todas as modificacoes estao marcadas com a palavra Viasoft.

Modificacao feita por Roberto - Viasoft em 09.06.2009
  O Path default de gravacao foi alterado de WinSys para Temp, de modo a evitar
  o erro que frequentemente ocorria por o usuario nao ter direito de gravacao
...
uses
  Types,       // Sytem.Types (included for inline expansion)
...
begin
  // replace constants
  if Pos(AnsiUpperCase(TppFilePathVariables.WinSys), AnsiUpperCase(aStorageName)) > 0 then
    // backwards compatibility, resolve ($WinSys) to Windows directory
    //Viasoft - linha abaixo comentada e substituida pela seguinte em 22.02.2011
    //aStorageName := StringReplace(aStorageName, TppFilePathVariables.WinSys, TppFileUtils.GetWindowsFilePath, [rfIgnoreCase])
    aStorageName := DefaultStorageName;
